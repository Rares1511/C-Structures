...
vector *vector_initiate ( size_t dim ) { 
...
size_t vector_find ( vector *vec, void *el ) {
    void *aux = malloc ( vec->dim );
    for ( size_t i = 0; i < vec->size; i++ ) {
        aux = vec->vec + i * vec->dim;
        if ( universal_compare ( aux, el, vec->dim ) == 0 )
            return i;
    }
    return -1;
}
...
/* sorts the vector using vecsort and the compare function offered */
void vector_sort ( vector *vec, comparer compare ) { 
    qsort ( vec->vec, vec->size, vec->dim, compare ); 
}
