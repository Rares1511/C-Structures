...
cs_codes vector_init ( vector *vec, vector_attr_t attr ) { 
    if ( attr.size < 0 || attr.size > 1000 ) return CS_SIZE;
...
cs_codes vector_push_back ( vector *vec, void *el ) { 
    ...
    memcpy ( vec->vec + vec->size * vec->attr.size, el, vec->attr.size );
    vec->size++;
    return CS_SUCCESS;
}

cs_codes vector_erase ( vector *vec, int pos ) {
    if ( vec->size == 0 ) return CS_EMPTY;
    if ( pos >= vec->size ) return CS_POS;
    if ( vec->attr.fr ) vec->attr.fr ( vec->vec + vec->attr.size * pos );
    if ( pos != vec->size - 1 )
