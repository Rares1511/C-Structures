...
void *large_number_mul_helper_thread(void *arg) {
    ...
    *max_size = 0;
    ...
    for (i = ln_thread_arg.thread_id; i < ln_thread_arg.ln1->size + ln_thread_arg.ln2->size;
         i += ln_thread_arg.thread_count) {
        ...
        for (j = jmin; j <= jmax; j++) {
            ...
            *max_size = *max_size > i ? *max_size : i;
            ln_thread_arg.ln1->aux1->vec[i] +=
                ln_thread_arg.ln1->vec[j] * ln_thread_arg.ln2->vec[k];
        }
    }
    (*max_size)++;
...
cs_codes large_number_mul_helper(large_number *ln1, large_number ln2) {
    ...
    long number_of_processors =
        sysconf(_SC_NPROCESSORS_ONLN) < ln1->size ? sysconf(_SC_NPROCESSORS_ONLN) : ln1->size;
