...
{
    if (fst_attr.size < 0 || snd_attr.size < 0 || fst_attr.size > SIZE_TH || snd_attr.size > SIZE_TH)
        return CS_SIZE;
    p->fst = NULL;
    p->snd = NULL;
    p->fst_attr = fst_attr;
...
void pair_set_fst_attr(pair *p, pair_attr_t fst_attr) { p->fst_attr = fst_attr; }

void pair_set_snd_attr(pair *p, pair_attr_t snd_attr) { p->snd_attr = snd_attr; }
...
void pair_set_snd_comp(pair *p, comparer snd_comp) { p->snd_attr.comp = snd_comp; }

void pair_set_both_comp(pair *p, comparer fst_comp, comparer snd_comp)
{
    p->fst_attr.comp = fst_comp;
    p->snd_attr.comp = snd_comp;
}
...
{
    if (p1->fst_attr.size != p2->fst_attr.size || p1->snd_attr.size != p2->snd_attr.size)
        return CS_SIZE;
    pair paux;
...
int pair_compare(pair p1, pair p2)
{
    if (p1.fst_attr.size != p2.fst_attr.size || p1.snd_attr.size != p2.snd_attr.size)
        return 0;
    if (!p1.fst_attr.comp || !p1.snd_attr.comp || !p2.fst_attr.comp || !p2.snd_attr.comp)
        return 0;
    int fst_res = (p1.fst_attr.comp(p1.fst, p2.fst) + p2.fst_attr.comp(p1.fst, p2.fst)) / 2;
    if (fst_res == 0)
        return (p1.snd_attr.comp(p1.snd, p2.snd) + p2.snd_attr.comp(p1.snd, p2.snd)) / 2;
    return fst_res;
}