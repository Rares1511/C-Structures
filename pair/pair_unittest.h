...
test_res test_pair_init(test_arg *arg) {
    ...
    p = pair_init(&first_attr, &second_attr);
    if (p == NULL) {
        clogger_log(*arg->logger, CLOGGER_ERROR, "pair_init failed\n");
        return (test_res){(char*)__func__, "pair_init failed", CS_UNKNOWN};
    }
...
test_res test_pair_init_different_types(test_arg *arg) {
    ...
    p = pair_init(&first_attr, &second_attr);
    if (p == NULL) {
        clogger_log(*arg->logger, CLOGGER_ERROR, "pair_init with different types failed\n");
        return (test_res){(char*)__func__, "pair_init with different types failed", CS_UNKNOWN};
    }
...
test_res test_pair_init_null_first_attr(test_arg *arg) {
    ...
    p = pair_init(NULL, &second_attr);
    if (p != NULL) {
        clogger_log(*arg->logger, CLOGGER_ERROR, "Expected NULL but got %p\n", p);
        return (test_res){(char*)__func__, "pair_init with NULL first_attr should return NULL", CS_UNKNOWN};
    }
    clogger_log(*arg->logger, CLOGGER_DEBUG, "Correctly returned NULL for NULL first_attr\n");
...
test_res test_pair_init_null_second_attr(test_arg *arg) {
    ...
    p = pair_init(&first_attr, NULL);
    if (p != NULL) {
        clogger_log(*arg->logger, CLOGGER_ERROR, "Expected NULL but got %p\n", p);
        return (test_res){(char*)__func__, "pair_init with NULL second_attr should return NULL", CS_UNKNOWN};
    }
    clogger_log(*arg->logger, CLOGGER_DEBUG, "Correctly returned NULL for NULL second_attr\n");
...
test_res test_pair_init_zero_first_size(test_arg *arg) {
    ...
    if (p != NULL) {
        clogger_log(*arg->logger, CLOGGER_ERROR, "Expected NULL but got %p\n", p);
        return (test_res){(char*)__func__, "pair_init with zero first size should return NULL", CS_UNKNOWN};
    }
    clogger_log(*arg->logger, CLOGGER_DEBUG, "Correctly returned NULL for zero first size\n");
...
test_res test_pair_init_zero_second_size(test_arg *arg) {
    ...
    if (p != NULL) {
        clogger_log(*arg->logger, CLOGGER_ERROR, "Expected NULL but got %p\n", p);
        return (test_res){(char*)__func__, "pair_init with zero second size should return NULL", CS_UNKNOWN};
    }
    clogger_log(*arg->logger, CLOGGER_DEBUG, "Correctly returned NULL for zero second size\n");
...
test_res test_pair_multiple_pairs(test_arg *arg) {
    ...
    for (int i = 0; i < 50; i++) {
        pairs[i] = pair_init(&first_attr, &second_attr);
        if (!pairs[i]) {
            clogger_log(*arg->logger, CLOGGER_ERROR, "pair_init failed at index %d\n", i);
