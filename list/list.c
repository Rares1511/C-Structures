...
list_node *list_node_init(void *el, int size, deepcopy cp)
{
...
{
    list_node *aux = list_node_init(el, l->attr.size, l->attr.cp);
    if (!aux)
        return CS_MEM;
    if (l->size != 0)
    {
