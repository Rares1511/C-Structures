...
int stack_empty ( stack s ) { if ( s.size == 0 ) return 1; return 0; }
...
cs_codes stack_pop ( stack *s ) {
    if ( stack_empty ( *s ) ) return CS_EMPTY;
    if ( s->attr.fr ) s->attr.fr ( s->vec + ( s->size - 1 ) * s->attr.size );
    s->size--;
    return CS_SUCCESS;
}
