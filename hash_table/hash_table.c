...
enum return_codes hash_table_delete_entry ( hash_table *ht, void *el, comparer comp ) {
    size_t table_pos = ht->hf  ( el );
    if ( table_pos > ht->size ) return WRONG_ELEMENT;
    size_t pos = vector_find ( &ht->table[table_pos], el, comp );
    if ( pos == ht->table[table_pos].size + 1 ) return WRONG_ELEMENT;
    return vector_erase ( &ht->table[table_pos], pos );
}
