...
deque_node *deque_node_init(void *data, int size, deepcopy cp)
{
...
{
    deque_node *node = deque_node_init(el, dq->attr.size, dq->attr.cp);
    if (!node)
        return CS_MEM;
    if (dq->size == 0)
    {
